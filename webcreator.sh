...
if [ $3 -eq $3 ] 2>/dev/null && [ $3 -gt 2 ] 2>/dev/null; then
...
if [ $4 -eq $4 ] 2>/dev/null && [ $4 -gt 2 ] 2>/dev/null; then
...
for ((w=0; w<"$3"; w++)) do
	...
	while [[ "$f_temp_i" -lt "$f" ]] || [[ "$q_temp_i" -lt "$q" ]]; do
		...
		if [ $[$RANDOM%2] -eq 0 ] ; then		#internal links only
		    ...
		    echo "<a href='"${intlinks[$f_temp]:14}"'>"${intlinks[$f_temp]}"_text</a>" >> $FILE
		...
		if [ $[$RANDOM%2] -eq 1 ] ; then		#external links only
		    ...
		    echo "<a href='"${extlinks[$q_temp]:14}"'>"${extlinks[$q_temp]}"_text</a>" >> $FILE
								#write the random link to html
		    k=$[$k+$nol+1]				#next writing must begin from line k
